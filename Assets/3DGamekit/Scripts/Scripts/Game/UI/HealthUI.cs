    ...
    public class HealthUI : MonoBehaviour
    {
        ...
        public void ChangeHitPointUI(Damageable damageable)
        {
            ...
            for (int i = 0; i < m_HealthIconAnimators.Length; i++)
            {
                ...
                AkSoundEngine.SetRTPCValue(AudioSys.GetWwiseEventName(EllenWwiseEvent.PlayerHealth) , damageable.currentHitPoints); //返回，血量值1-5，rtpc
                //Debug.Log(damageable.currentHitPoints);
                //PlayerHealth.SetGlobalValue(damageable.currentHitPoints);
            }
        }
    } 
}
