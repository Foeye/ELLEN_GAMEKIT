        ...
        private bool CheckDamage(Collider other, AttackPoint pts)
        {
            ...
            if (hitAudio != null)
            {
                ...
                if (renderer)
                {
                    ...
                    //* Box hit sound 的触发放到 Dissolve.cs 里去了
                    if (renderer.sharedMaterial.ToString().Substring(0,3) == "Box")
                    {
                        AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Destructible_Boxes_Hit), gameObject);//Box hit sound,会触发多次
                    }
                    Debug.Log(renderer.sharedMaterial.ToString().Substring(0,3));
                    //*/

                    if (renderer.sharedMaterial.ToString().Substring(0,3) == "Cho")
                    {
                        Debug.Log(renderer.sharedMaterial);
                        AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Chomper_Hit), gameObject);//Chomper hit sound
                    }
                }
                else
                {
                    ...
                    AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Grenadier_Hit), gameObject);// boss hit 
                }
            }
