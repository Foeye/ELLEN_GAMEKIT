...
namespace Gamekit3D
{
    ...
    public class PlayerController : MonoBehaviour, IMessageReceiver
    {
        ...
        public string OldMat; //用于储存上一帧脚步材质
        public string CurMat; //用于储存当前脚步材质
        ...
        readonly EllenWwiseEvent[] StickAttackCombo = new EllenWwiseEvent[] { 
            EllenWwiseEvent.Weapon_Stick_Swipe_Sub_1, 
            EllenWwiseEvent.Weapon_Stick_Swipe_Sub_2, 
            EllenWwiseEvent.Weapon_Stick_Swipe_Sub_3, 
            EllenWwiseEvent.Weapon_Stick_Swipe_Sub_4
        };
        ...
        void PlayAudio()
{
    ...
    if (m_CurrentWalkingSurface == null) {
        ...
        CurMat = m_CurrentWalkingSurface.ToString().Substring(0,5); //脚步切换材质的前5个字母进行比对
    }

    if(CurMat != OldMat)
    {
        if (CurMat == "Floor" || CurMat == "Stair")
        {
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Set_FootStep_Switch_Rock), gameObject);
        }

        else if (CurMat == "Drops" )
        {
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Set_FootStep_Switch_Metal), gameObject);
        }

        else
        {
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Set_FootStep_Switch_Grass), gameObject);
        }
    ...
    if (footfallCurve > 0.01f && !footstepPlayer.playing && footstepPlayer.canPlay)
    {
        ...
        AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Ellen_Footstep_Walk), gameObject); //Ellen脚步声
    }
    ...
    {
        AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Ellen_Footstep_Landing), gameObject); //Ellen落地声
    }

    if (!m_IsGrounded && m_PreviouslyGrounded && m_VerticalSpeed > 0f)
    {
        emoteJumpPlayer.PlayRandomClip();
        AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Ellen_Footstep_Jump), gameObject); //Ellen跳跃声
    }

    if (m_CurrentStateInfo.shortNameHash == m_HashHurt && m_PreviousCurrentStateInfo.shortNameHash != m_HashHurt)
    {
        hurtAudioPlayer.PlayRandomClip();
        AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Ellen_Hurt_Vox), gameObject); //Ellen受击
    }
    ...
    if (m_CurrentStateInfo.shortNameHash == m_HashEllenDeath && m_PreviousCurrentStateInfo.shortNameHash != m_HashEllenDeath)
    {
        ...
        AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Music_Death), gameObject); //Ellen死亡
        AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Ellen_Death_Vox), gameObject); //Ellen死亡
    }
        ...
        m_CurrentStateInfo.shortNameHash == m_HashEllenCombo4 && m_PreviousCurrentStateInfo.shortNameHash != m_HashEllenCombo4)
    {
        ...
        AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Ellen_Attack_Vox), gameObject); //Ellen攻击
    }
}

        // 3个 Landing Animation 音效及语音开关，用于放入动画的 Animation Event
        void LandFastIdleSound()
        {
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Set_Landing_Switch_Fast_Idle), gameObject);
        }

        void LandRunSound()
        {
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Set_Landing_Switch_Run), gameObject);
        }

        void LandFastRunSound()
        {
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Set_Landing_Switch_Fast_Run), gameObject);
        }
        ...
        // Idle Animation 01 音效及语音，用于放入动画的 Animation Event
        void EllenIdle01Sound()
        {
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Ellen_Idle_01), gameObject);
        }

        void EllenIdle02Sound()
        {
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Ellen_Idle_02), gameObject);
        }

        void EllenIdle03Sound()
        {
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Ellen_Idle_03), gameObject);
        }
        ...
        public void MeleeAttackStart(int throwing = 0)
        {
            ...
            var curAttackEnum = EllenWwiseEvent.Weapon_Stick_Swipe_Sub_1;
            if (throwing > 0 && throwing <= StickAttackCombo.Length) {
                curAttackEnum = StickAttackCombo[throwing - 1];
            }
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(curAttackEnum), gameObject); //空挥武器声音
        ...
        protected IEnumerator RespawnRoutine()
        {
            ...
            AkSoundEngine.PostEvent(AudioSys.GetWwiseEventName(EllenWwiseEvent.Music_Respawn), gameObject); //重生音乐
